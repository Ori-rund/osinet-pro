...
async def run() -> None:
    ...
    # ערוצים שכבר ניסינו להצטרף אליהם בהרצה הזו — כדי לא לחזור על זה
    # בכל רענון (כל 5 דק'). לא נטען מ-get_dialogs(): לחשבון הזה כבר
    # יש דיאלוגים רבים (ראה "In-memory entity cache" בלוגים), ו-
    # get_dialogs() המלא (מתפרס על כמה בקשות) נתקל בפועל ב-FloodWait
    # חוזר ומוסיף כמה דקות לכל עלייה. JoinChannelRequest על ערוץ
    # שכבר הצטרפת אליו זול ובטוח (מחזיר הצלחה בלי פעולה בפועל), אז
    # במקום לבדוק קודם, פשוט מנסים להצטרף לכל ערוץ פעם אחת בהרצה.
