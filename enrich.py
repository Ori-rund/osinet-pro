...
_EMOJI = re.compile(
    "[\U0001F000-\U0001FAFF\U00002600-\U000027BF\U0001F1E6-\U0001F1FF←-⇿⬀-⯿︎️‍]",
    flags=re.UNICODE,
)
...
_BOILERPLATE = [
    ...
    # דלף בפועל: "-- חדשות השומרון -- וואטסאפ did.li/... טלגרם
    # did.li/... ערוץ did.li/..." — הכיתוב אחרי המקף הכפול משתנה
    # (איות "וואטסאפ"/"ווטסאפ", ולינקים נוספים ל"טלגרם"/"ערוץ" אחרי
    # ה-וואטסאפ), אז עוגן $ מיד אחרי מילה בודדת לא תפס יותר. המקף
    # הכפול "-- חדשות השומרון --" עצמו הוא תמיד חתימה, לא תוכן —
    # מסירים ממנו ועד סוף ההודעה, מה שנשאר אחריו.
    r"--\s*חדשות\s*השומרון\s*--[\s\S]*$",
    # חתימת ערוץ "הלל ביטון רוזן" — שם הערוץ בסוף כל הודעה.
    r"הלל ביטון רוזן בטלגרם\s*$",
    # חתימת ערוץ "צופר" (t.me/tzevaadomm) — פרסומת עצמית בסוף כל
    # הודעה, לא חלק מתוכן ההתרעה.
    r"צופר\s*-\s*צבע אדום\s*$",
    # פורמט ההודעות של "צופר" (משדר את פיקוד העורף כמעט מילה במילה):
    # "<קטגוריה> (17/09/2026): 14:33: • <אזור>: <ישובים> (15 שניות)".
    # התאריך/שעה מיותרים — האתר כבר מציג זמן קליטה משלו (occurred_at),
    # וזמן ההגעה לממד לא רלוונטי למי שלא שם. לא חתימה בסוף (`$`),
    # אלה שני קטעים באמצע ההודעה, לכן בלי עוגן סוף-שורה.
...
_STOPWORDS = {
    ...
    # דלף בפועל: "זמן" הוא כל מה שחפף בין עדכון על ג'נין לעדכון
    # לגמרי לא קשור על עזה ("לפני זמן קצר", "בטווח הזמן המיידי") —
    # אותה מחלת "זנב משותף" כמו "נפגעים" למעלה, רק עם "זמן"/"קצר".
    "זמן", "קצר",
    # דלף בפועל (64432cc2): "חדשות"/"חדש" חפף בין עדכון על מטען נגד
    # כלי הנדסי בעזה לעדכון לגמרי לא קשור על תפיסת מחבל ביו"ש —
    # אבל לא כי שני האירועים דומים, אלא כי שם הערוץ שהתמזג לתוכן
    # ("עדכון (חדשות לפני כולם, ...)") חפף במקרה עם "חדשות 14" בטקסט
    # של האירוע השני. "חדש/ה/ות" הוא מילוי עיתונאי/שם ערוץ גנרי,
    # אותה מחלה בדיוק כמו "נפגעים"/"זמן" למעלה.
    "חדש",
}
