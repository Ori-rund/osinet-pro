...
OVERLAP_CASES = [
    ...
    # דלף בפועל (64432cc2): עדכון על מטען נגד כלי הנדסי בעזה התאחד
    # עם עדכון לגמרי לא קשור על תפיסת מחבל רוצח ביו"ש — כי "עדכון
    # (חדשות לפני כולם, ...)" (שם הערוץ שנדבק לתוכן הממוזג) חפף עם
    # "מהדורת חדשות 14" בטקסט השני. "חדשות"/"חדש" עכשיו במילות העצירה.
    ('חמאס מנסה לפגוע בכוחותינו: לפני זמן קצר הופעל מטען לעבר כלי הנדסי של צה"ל שפעל בצפון רצועת עזה. אין נפגעים.\n'
     'עדכון (חדשות לפני כולם, 21:33:09): צה"ל תוקף כעת בעזה אחרי פיצוץ מטען בצפון הרצועה',
     'ככה נתפס המחבל: התקשר לאשתו לבשר לה שרצח יהודי אליה אביב חושף במהדורת חדשות 14 | פרטים נוספים על הפיגוע מיום ראשון, '
     'וגם - איך הצליח צה"ל לאתר את המחבל במהירות למרות שנמלט קילומטרים רבים? צפו בדיווח המלא >>',
     "", False, "עזה מול מחבל ביו\"ש — לא קשור, חפיפה גנרית בלבד ('חדשות')"),
]
