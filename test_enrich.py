...
OVERLAP_CASES = [
    ...
    # דלף בפועל: עדכון על מעצר מחבלים בג'נין (עם מיקום) התאחד עם
    # עדכון לגמרי לא קשור על מטען שהופעל על כלי הנדסי בעזה (בלי
    # מיקום) — כי "זמן"/"קצר" (מ"לפני זמן קצר"/"בטווח הזמן המיידי")
    # היו החפיפה היחידה, ומילים גנריות-זמניות כאלה לא אמורות
    # להספיק לבדן. שתיהן עכשיו במילות העצירה.
    ("לוחמי צה\"ל עצרו סמוך לג'נין 2 מחבלים שתכננו פיגוע בזמן יום כיפור",
     "הטרור הערבי בעזה: לפני זמן קצר, מחבלי חמאס הפעילו מטען נפץ על כלי הנדסי בצפון הרצועה. בנס אין נפגעים לכוחותינו.",
     "ג'נין", False, "ג'נין מול עזה — לא קשור, חפיפה גנרית בלבד ('זמן')"),
]
...
def main() -> int:
    ...
    # דלף בפועל: אותה חתימה, אבל עם איות "וואטסאפ" (לא "ווטסאפ") וגם
    # שלושה לינקים נוספים (טלגרם/ערוץ) אחריה — $ מיד אחרי מילה בודדת
    # לא תפס יותר, כי יש עוד תוכן חתימה אחרי ה-וואטסאפ.
    binyamin_sig_links = enrich(
        "אריאל סיום אירוע באזור המרכז המסחרי. הדרך נפתחה לתנועה בשני "
        "הכיוונים. -- חדשות השומרון -- וואטסאפ did.li/shomron-news "
        "טלגרם did.li/shomron-news-TG ערוץ did.li/shomron-news-Channel")
    assert "חדשות השומרון" not in binyamin_sig_links["content"], "חתימה עם איות/לינקים שונים לא נוקתה"
    assert "did.li" not in binyamin_sig_links["content"], "לינקי החתימה לא הוסרו"
